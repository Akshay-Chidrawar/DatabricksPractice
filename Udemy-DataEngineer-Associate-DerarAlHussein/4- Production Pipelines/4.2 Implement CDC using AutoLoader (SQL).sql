...
-- MAGIC %md
-- MAGIC #### Bronze (streaming data for CDC feed for Books)

-- COMMAND ----------

CREATE OR REFRESH STREAMING LIVE TABLE books_cdc
COMMENT "The raw books data, ingested from CDC feed"
AS 
SELECT  * 
FROM    cloud_files("${datasets.path}/books-cdc", "json")
...
-- MAGIC %md
-- MAGIC #### Silver (Apply above CDC changes on target streaming table)
...
FROM  (
      ...
      INNER JOIN LIVE.books_silver b ON o.book.book_id = b.book_id;
